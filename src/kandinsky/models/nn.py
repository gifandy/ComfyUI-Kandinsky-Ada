...
class VisualEmbeddings(nn.Module):
    ...
    def forward(self, x):
        if x.dim() == 5:
            batch, duration, height, width, dim = x.shape
            x = x.squeeze(0)
        else:
...
class MultiheadCrossAttention(nn.Module):
    ...
    def attention(self, query, key, value, attention_mask=None):
        if self.attn_engine.get_attention().__name__ == 'sage_attn':
            out = self.attn_engine.get_attention()(q=query.unsqueeze(0), k=key, v=value)[0].flatten(-2, -1)
        else:
            out = self.attn_engine.get_attention()(q=query.unsqueeze(0), k=key, v=value, attn_mask=attention_mask)[0].flatten(-2, -1)
        return out
    ...
    def forward(self, x, cond, attention_mask=None):
        ...
        out = self.attention(query, key, value, attention_mask=attention_mask)
        out = self.out_l(out)
        return out
