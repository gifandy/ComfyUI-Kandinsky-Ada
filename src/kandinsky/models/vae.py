...
class AutoencoderKLHunyuanVideo(ModelMixin, ConfigMixin):
    ...
    def decode(
        ...
        Returns:
            [`~models.vae.DecoderOutput`] or `tuple`:
                ...
                otherwise a plain `tuple` is returned.
        """
        # Only calculate optimal tiling if not already manually configured
        # Check if tile parameters have been manually set (not using defaults)
        manually_configured = (
            hasattr(self, 'tile_sample_min_height') and
            hasattr(self, 'tile_sample_min_width') and
            self.tile_sample_min_height not in [256, 0] and
            self.tile_sample_min_width not in [256, 0]
        )
    ...
    def get_enc_optimal_tiling(
        ...
        if (sqrt(height * width) < 450) and (num_frames <= 97):
            ...
            # Use optimal tiling if available, otherwise use full frames
        ...
        if sqrt(height * width) > 500:
            # Use lookup table if exact match, otherwise scale from closest
            if height in OPT_SPATIAL_TILING:
                ht, hs = OPT_SPATIAL_TILING[height]
            else:
                # Find closest and scale proportionally to actual dimension
                closest_h = min(OPT_SPATIAL_TILING.keys(), key=lambda k: abs(k - height))
                base_tile_h, base_stride_h = OPT_SPATIAL_TILING[closest_h]
                # Scale to maintain same coverage ratio
                scale = height / closest_h
                ht = int(base_tile_h * scale)
                hs = int(base_stride_h * scale)

            if width in OPT_SPATIAL_TILING:
                wt, ws = OPT_SPATIAL_TILING[width]
            else:
                # Find closest and scale proportionally to actual dimension
                closest_w = min(OPT_SPATIAL_TILING.keys(), key=lambda k: abs(k - width))
                base_tile_w, base_stride_w = OPT_SPATIAL_TILING[closest_w]
                # Scale to maintain same coverage ratio
                scale = width / closest_w
                wt = int(base_tile_w * scale)
                ws = int(base_stride_w * scale)
        else:
