...
class GGMLTensor(torch.Tensor):
    ...
    def shape(self):
        if not hasattr(self, "tensor_shape"):
            self.tensor_shape = super().size()
        return self.tensor_shape

    @property
    def dtype(self):
        """Report floating point dtype for gradient compatibility"""
        # Even though underlying storage is uint8, we need to report
        # a float dtype for PyTorch to allow this as a parameter
        return torch.float16

    def size(self, dim=None):
        """Override size() to return logical shape, not quantized byte array shape"""
        if dim is not None:
            return self.shape[dim]
        return self.shape

    def numel(self):
        """Override numel() to return logical number of elements"""
        import math
        return math.prod(self.shape)

    def dim(self):
        """Override dim() to return number of dimensions in logical shape"""
        return len(self.shape)

    @property
    def requires_grad(self):
        """Quantized tensors don't support gradients"""
        return False

    @requires_grad.setter
    def requires_grad(self, value):
        """Ignore attempts to set requires_grad on quantized tensors"""
        pass

    def requires_grad_(self, value=True):
        """In-place version - ignore for quantized tensors"""
        return self
